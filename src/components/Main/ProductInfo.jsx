...
ProductInfo.Quantity = function ProductInfoQuantity({quantity, setQuantity}) {
  return (
    <div className="p-2 rounded-[10px] bg-grey-50 flex justify-between flex-1">
      <button onClick={() => setQuantity(prevQuantity => prevQuantity - 1)}><img src={minusIcon} alt=""/></button>
      <p className="text-preset-3-bold text-grey-950">{quantity}</p>
      <button onClick={() => setQuantity(prevQuantity => prevQuantity + 1)}><img src={plusIcon} alt=""/></button>
    </div>
  )
}
...
ProductInfo.AddToCart = function ProductInfoAddToCart(){
  return(
    <button className="p-2 flex justify-center gap-2 bg-orange-500 rounded-[10px] flex-1">
