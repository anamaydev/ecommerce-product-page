...
import {CartDataContext, ProductDataContext} from "../../App.jsx";
...
ProductInfo.Quantity = function ProductInfoQuantity({quantity, setQuantity}) {
  return (
    <div className="p-2 rounded-[10px] bg-grey-50 flex justify-between flex-1">
      <button className="cursor-pointer active:scale-95" onClick={() => setQuantity(prevQuantity => prevQuantity === 0 ? 0 : prevQuantity - 1)}><img src={minusIcon} alt=""/></button>
...
ProductInfo.AddToCart = function ProductInfoAddToCart({quantity}){
  const data = useContext(ProductDataContext);
  ...
  function addToCart(){
    setCart({
      ...
      quantity: quantity,
      image: {
        src: data.productImages[0].src,
        alt: data.productImages[0].alt
      },
  ...
  return(
    <button
      className="p-2 flex justify-center gap-2 bg-orange-500 hover:bg-orange-300 rounded-[10px] flex-1 cursor-pointer active:scale-95"
      onClick={addToCart}
    >
